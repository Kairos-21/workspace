    ...
    document.getElementById('folderIconFile').onchange = async (e) => {
        ...
        try {
            ...
            if (data.success && data.data) {
                customIconPath = data.data.path;
                preview.innerHTML = `
                    <div class="uploaded-icon-wrapper">
                        <img src="${data.data.path}" class="uploaded-icon-img">
...
function showAddShortcutModal() {
    ...
    function tryFetchFavicon() {
        ...
        // 多个备选 favicon 服务（Google 优先，最稳定）
        const fallbackServices = [
            'https://www.google.com/s2/favicons?domain=' + hostname + '&sz=64',
            'https://icon.horse/icon/' + hostname,
            'https://favicone.com/' + hostname + '?s=64',
        ...
        // 尝试加载 favicon，失败则尝试下一个服务
        function tryNextService() {
            if (currentIndex >= fallbackServices.length) {
                preview.innerHTML = '<span style="color: var(--text-secondary); font-size: 0.85rem;">❌ 抓取失败，可选择手动上传</span>';
                return;
            }
            
            const faviconUrl = fallbackServices[currentIndex];
            console.log('[Favicon] 尝试服务:', faviconUrl);
            
            const img = document.createElement('img');
            img.style.cssText = 'width:48px;height:48px;border-radius:8px;object-fit:contain;';
            img.src = faviconUrl;
            ...
            img.onload = () => {
                ...
                console.log('[Favicon] 成功:', faviconUrl);
            };
            
            img.onerror = () => {
                console.log('[Favicon] 失败，尝试下一个服务');
                currentIndex++;
                tryNextService();
            };
        }
        ...
        fetch(`/api/favicon?url=${encodeURIComponent(url)}`)
            ...
            .then(data => {
                if (data.success && data.data && data.data.faviconUrl) {
                    ...
                    img.onerror = () => {
                        // 后端返回的也失败，尝试备用服务
                        console.log('[Favicon] 后端返回的 URL 失败，尝试备用服务');
                        tryNextService();
                    };
                } else {
                    // 后端失败，直接尝试备用服务
            ...
            .catch(err => {
                console.error('获取 favicon 失败，尝试备用服务:', err);
                tryNextService();
            });
    }
    ...
    if (uploadBtn && fileInput) {
        ...
        fileInput.onchange = async (e) => {
            ...
            try {
                ...
                if (data.success && data.data) {
                    currentCustomIconPath = data.data.path;
                    preview.innerHTML = `<img src="${data.data.path}" style="width:48px;height:48px;border-radius:8px;object-fit:contain;">`;
...
function deleteShortcut(id) {
    ...
    if (index > -1) {
        ...
        if (shortcut.iconType === 'custom' && shortcut.customIconPath) {
            fetch(`/api/file?path=${encodeURIComponent(shortcut.customIconPath)}&type=icon`, {
                method: 'DELETE'
            }).catch(err => console.error('删除图标文件失败:', err));
        }
    ...
    for (const folder of shortcuts.filter(s => s.type === 'folder')) {
        if (folder.children) {
            ...
            if (index > -1) {
                const shortcut = folder.children[index];
                if (shortcut.iconType === 'custom' && shortcut.customIconPath) {
                    fetch(`/api/file?path=${encodeURIComponent(shortcut.customIconPath)}&type=icon`, {
                        method: 'DELETE'
                    }).catch(err => console.error('删除图标文件失败:', err));
